...
The repo creates a library that defines custom QML types defined in C++ and QML, and use that library in a simple NFT game.
...
This will make available our target library at configuration time and build it.

To make available our target library once is compiled is better to use the CMake [`install`](https://cmake.org/cmake/help/latest/command/install.html) function.
...
An example of the use of this type is present  in the `NFTMonitor.qml` file.
In this case, the `NFTMonitor` type is [defined in a QML document](https://doc.qt.io/qt-6/qtqml-documents-definetypes.html) and can be seen as a higher-level type of our `CPPMonitor`.
